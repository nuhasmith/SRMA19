// ============================================================
//  BERITA_HUMAS.JS – Manajemen Berita untuk Role Humas
//  SRMA 19 Bantul
//  Versi: 2.2.0 - Final Fix, Semua Fungsi Global & Modul Tersedia
// ============================================================

// ---- GLOBAL STUBS (Dijalankan Sebelum Modul, Mencegah ReferenceError) ----
...
window.selectMediaFromLibrary = function(url) {};
window.uploadGambar = function() {};
window.hapusDraft = function() {};
window.toggleFullScreenEditor = function() {};
window.toggleLivePreviewPanel = function() {};
window.updateLivePreview = function() {};
window.switchViewMode = function() {};
window.applyFilter = function() {};
window.resetFilter = function() {};
window.showEdit = function() {};
window.showBeritaModal = function() {};
...
(function() {
    ...
    async function openMediaLibrary() {
        ...
        if (driveImagesCache.length === 0) {
            ...
            driveImagesCache.forEach(img => {
                html += `<div class="col-3 col-md-2"><div class="card h-100 cursor-pointer" onclick="selectMediaFromLibrary('${img.url}')" style="cursor:pointer;transition:0.2s;"><img src="${img.url}" class="card-img-top" style="height:100px;object-fit:cover;"><div class="card-body p-1 text-center small text-truncate" title="${img.name}">${img.name}</div></div></div>`;
            });
        }
        html += `</div></div></div>`;
    ...
    // ============================================================
    function renderBeritaHumas(container) {
        ...
        container.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    <button class="btn btn-sm btn-outline-primary rounded-pill ${beritaViewMode === 'list' ? 'active' : ''}" onclick="switchViewMode('list')">
                        <i class="fas fa-list me-1"></i> Mode Daftar
                    </button>
                    <button class="btn btn-sm btn-success rounded-pill ${beritaViewMode === 'write' ? 'active' : ''}" onclick="switchViewMode('write')">
                        <i class="fas fa-pen-fancy me-1"></i> Mode Tulis Cepat
                    </button>
                    <button class="btn btn-sm btn-refresh rounded-pill" onclick="refresh(false)"><i class="fas fa-sync-alt me-1"></i> Refresh</button>
                </div>
            </div>
            <div class="card-modern mb-3">
                <div class="filter-group">
                    <input type="text" id="searchBerita" placeholder="🔍 Cari judul..." style="flex:1;min-width:150px;" oninput="applyFilter()">
                    <select id="filterStatusBerita" onchange="applyFilter()">
    ...
    function renderTable() {
        ...
        beritaFiltered.forEach((b, i) => {
            ...
            rows += `<tr>
                <td>${i+1}</td>
                <td><input type="checkbox" class="berita-checkbox" data-id="${b.ID}" ${selectedBeritaIds.has(b.ID)?'checked':''} onchange="toggleSelect(${b.ID}, this.checked)"></td>
                ...
                <td class="text-center">
                    <button class="btn btn-sm btn-outline-primary p-1" onclick="showEdit(${realIdx})" title="Edit"><i class="fas fa-edit"></i></button>
                    <button class="btn btn-sm btn-outline-info p-1" onclick="lihatRiwayat(${realIdx})" title="Riwayat"><i class="fas fa-history"></i></button>
                    <button class="btn btn-sm btn-outline-danger p-1" onclick="hapus(${realIdx})" title="Hapus"><i class="fas fa-trash-alt"></i></button>
                </td>
            </tr>`;
        });
        ...
        document.getElementById('beritaTableContainer').innerHTML = `
            <div class="p-3 border-bottom bg-light d-flex align-items-center gap-2 flex-wrap">
                ...
                <button class="btn btn-sm btn-primary rounded-pill" onclick="executeBulkAction()">Terapkan</button>
                <button class="btn btn-sm btn-outline-secondary rounded-pill" onclick="toggleSelectAll()">Pilih Semua</button>
    ...
    function showBeritaModal(index = null) {
            ...
            <div class="modal-overlay" id="beritaModal">
                <div class="modal-box" style="max-width: 1200px; width: 95%; padding: 0;">
                    <div class="modal-header-area">
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            <button class="btn btn-sm btn-outline-secondary rounded-pill" id="fsToggleBtn" onclick="toggleFullScreenEditor()" title="Mode Layar Penuh">
                        ...
                        <div class="row g-4" id="beritaEditorLayout">
                            <div class="col-md-7" id="editorColumn">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="bmJudul" value="${existing?.Judul || ''}" placeholder="Masukkan judul berita" oninput="updateSlugFromTitle(this.value)">
                                </div>
                                <div class="mb-2">
                            ...
                            <div class="col-md-5" id="sidebarColumn">
                                ...
                                <div class="wp-meta-box">
                                    ...
                                    <div class="wp-meta-box-body">
                                        ...
                                        <input type="file" id="bmGambarFile" accept="image/*" class="form-control form-control-sm mb-2" onchange="previewThumbnail(this)">
                                        <div class="d-flex gap-2">
                                            <button type="button" class="btn btn-sm btn-primary flex-fill" id="btnUploadGambar" onclick="uploadGambar()"><i class="fas fa-upload me-1"></i>Upload</button>
                                            <button type="button" class="btn btn-sm btn-outline-primary flex-fill" onclick="openMediaLibrary()"><i class="fas fa-folder-open me-1"></i>Media</button>
                                        </div>
                    ...
                    <div class="modal-footer-area">
                        <div class="d-flex align-items-center gap-3 flex-wrap flex-fill">
                            <button type="button" class="btn btn-sm btn-outline-danger rounded-pill" onclick="hapusDraft()" title="Hapus draft yang tersimpan">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button class="btn btn-save-wp" onclick="save(${index})"><i class="fas fa-save me-1"></i> Simpan</button>
                        </div>
                    </div>
                    <button type="button" id="previewCornerBtn" class="btn btn-primary rounded-circle shadow" style="position:fixed;bottom:24px;right:24px;width:52px;height:52px;z-index:2100;" onclick="toggleLivePreviewPanel()" title="Pratinjau Berita">
                    ...
                    <div id="livePreviewContainer" style="display:none;position:fixed;bottom:88px;right:24px;width:340px;max-height:60vh;overflow-y:auto;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:16px;box-shadow:0 10px 40px rgba(0,0,0,0.2);z-index:2100;">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button type="button" class="btn-close" style="font-size:0.7rem;" onclick="toggleLivePreviewPanel()"></button>
                        </div>
        ...
        // Override global functions dengan implementasi sebenarnya
        window.updateSlugFromTitle = function(title) {
    ...
    // ============================================================
    //  GLOBAL FALLBACK (Semua Fungsi Dipanggil Tanpa Prefix)
    // ============================================================
    window.switchViewMode = switchViewMode;
    window.applyFilter = applyFilter;
    window.resetFilter = resetFilter;
    window.showEdit = showEdit;
    window.showBeritaModal = showBeritaModal;
    window.save = save;
    window.hapus = hapus;
    ...
    window.lihatRiwayat = lihatRiwayat;
    window.hapusDraft = hapusDraft;
    window.uploadGambar = uploadGambar;
    window.openMediaLibrary = openMediaLibrary;
    window.selectMediaFromLibrary = selectMediaFromLibrary;
    window.previewThumbnail = previewThumbnail;
    window.updateSlugFromTitle = updateSlugFromTitle;
    window.generateChart = generateChart;
    window.insertChart = insertChart;
    window.toggleFullScreenEditor = toggleFullScreenEditor;
    window.toggleLivePreviewPanel = toggleLivePreviewPanel;
    window.updateLivePreview = updateLivePreview;

    console.log('✅ Humas Berita module loaded (final fix)');
})();